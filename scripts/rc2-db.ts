...
const QUERIES: Query[] = [
  { name: "workspaces.list_for_member", note: "membership fan-out",
  ...
  { name: "audit_events.by_workspace_recent", note: "governance dashboards",
    sql: "SELECT id, at, actor, action, entity_type, entity_id FROM public.audit_events WHERE workspace_id = '00000000-0000-0000-0000-000000000000'::uuid ORDER BY at DESC LIMIT 100" },
  { name: "audit_events.by_entity", note: "entity history drilldown",
    sql: "SELECT id, at, actor, action FROM public.audit_events WHERE entity_type = 'concepts' AND entity_id = 'demo' ORDER BY at DESC LIMIT 50" },
  { name: "knowledge_objects.by_workspace_status", note: "catalog filter",
    sql: "SELECT id, title, status FROM public.knowledge_objects WHERE workspace_id = '00000000-0000-0000-0000-000000000000'::uuid AND status = 'Canonical' LIMIT 100" },
  { name: "concepts.by_workspace", note: "registry list",
    sql: "SELECT id, name FROM public.concepts WHERE workspace_id = '00000000-0000-0000-0000-000000000000'::uuid LIMIT 200" },
  ...
  { name: "releases.by_workspace", note: "release list",
    sql: "SELECT id, stage, created_at FROM public.releases WHERE workspace_id = '00000000-0000-0000-0000-000000000000'::uuid ORDER BY created_at DESC LIMIT 50" },
  { name: "publications.by_workspace", note: "publications registry",
    sql: "SELECT id, title FROM public.publication_blueprints WHERE workspace_id = '00000000-0000-0000-0000-000000000000'::uuid" },
  { name: "review_items.open_by_workspace", note: "review queue",
    sql: "SELECT id FROM public.review_items WHERE workspace_id = '00000000-0000-0000-0000-000000000000'::uuid AND resolved = false LIMIT 100" },
